...
describe Widget::Data do
  ...
  context "when creating a widget" do
    ...
    context "and there isn't a widget with that name" do
      it {lambda{@data.create("Widgee 5")}.should change{@data.count}.by(1)}
      it { @data.create("Widgee 5").should be_true }

      context "but there is another widget" do
        before(:each){@data.create("Widgee 1") }
        it {@data.create("Widgee 5").should be_true }
        it {@data.create("Widgee 5"); @data.length.should == 2 }
      end
    end
    ...
    context "and there is already a widget with that name" do
      ...
      it "doesn't add a widget" do
        lambda{@data.create("Widgee 5")}.should_not change{@data.length}
      end
    end
  ...
  context "when deleteing widgets" do
    ...
    context "and you clear them all" do
      before(:each){@data.delete_all}
      it{@data.length.should == 0}
    end

    context 'and you delete one by name' do
      let(:widgee){"Widgee 0"}
      it {lambda{@data.delete(widgee)}.should change{@data.length}.from(10).to(9)}
      it {@data.delete(widgee); @data.read(widgee).should be_empty}
    end
